...
export class ConversationMemory {
    ...
    async getCompressedContext(maxTokens = 2000) {
        ...
        const shouldSummarize = session.messages.length > MAX_MESSAGES_BEFORE_SUMMARIZATION &&
    ...
    async summarizeHistory() {
        ...
        if (messagesToSummarize.length === 0)
            return;
        const { summary } = await summarizeConversation(messagesToSummarize.map(m => ({ role: m.role, content: m.content })), 0 // Summarize all
        );
