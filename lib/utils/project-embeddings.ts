...
import { chromaManager, ChromaMetadata, CodeChunk, generateChunkId } from '../db/chroma';
...
export class ProjectEmbeddingService {
  ...
  constructor(projectId: string, sourceDir: string = './generated') {
    this.projectId = projectId;
    this.sourceDir = sourceDir;
  }

  // Main method to embed entire project
  async embedProject(): Promise<void> {
  ...
  private async processCodeFiles(files: ProjectFile[]): Promise<CodeChunk[]> {
  ...
  private async createCodeChunks(file: ProjectFile): Promise<CodeChunk[]> {
    const chunks: CodeChunk[] = [];
    const lines = file.content.split('\n');
    const separators = CODE_SEPARATORS[file.language as keyof typeof CODE_SEPARATORS];
  ...
  private createLineBasedChunks(file: ProjectFile): CodeChunk[] {
    const chunks: CodeChunk[] = [];
    const lines = file.content.split('\n');
  ...
  private extractDependencies(content: string, language: string): string | null {
  ...
  private extractParentFunctions(content: string, language: string): string | null {
  ...
  private async createStructureEmbeddings(files: ProjectFile[]): Promise<Array<{
    id: string;
    content: string;
    metadata: any;
  }>> {
  ...
  private buildDirectoryTree(files: ProjectFile[]): string {
  ...
  private summarizeFileTypes(files: ProjectFile[]): string {
    const summary: { [key: string]: { count: number; totalSize: number } } = {};
  ...
  async embedConversation(sessionId: string): Promise<void> {
    try {
...
export async function embedProject(projectId: string, sourceDir: string = './generated'): Promise<void> {
...
export async function embedConversation(projectId: string, sessionId: string): Promise<void> {
